        ...
        d =>
          d[0] && (
            <div className="dotplot-group" key={d[0]}>
              ...
              <svg width={figureWidth} height="100%" aria-label="Dot Plot">
                {d[1]?.map((data, index) => (
                  <Spoke key={data.indicator_key} data={data} i={index} total={circleData.length} settings={settings} setTooltip={() => false} xPos={settings.xPos} />
                ))}
              </svg>
            </div>
          )
