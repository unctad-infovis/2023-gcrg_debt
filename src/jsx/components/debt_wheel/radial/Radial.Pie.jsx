import { arc, ascending, groups, pie } from 'd3';
...
function Pie({ settings }) {
  ...
  const slice_data = sliceGenerator(slice_entries).filter((_d, i) => slice_entries[i].id !== 'filler');

  const slices = pieData(slice_data, settings, settings.center ? 0.328 : 0.07, 1.18, 0).map((d, i) => ({
    d,
