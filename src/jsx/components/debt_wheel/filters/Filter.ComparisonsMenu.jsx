...
function Comparisons({ setCompOpen }) {
  ...
  const comparisonsG = groups(optionsList, d => d.category_display).map(d => ({
    group: d[0],
    values: d[1]
  }));
  ...
  const handleOnChange = position => {
    ...
    const numberChecked = updatedCheckedState.filter(d => d.checked === true).length;

    if (numberChecked <= 2) {
      const indices = updatedCheckedState.map(d => d.checked).reduce((out, bool, index) => (bool ? out.concat(index) : out), []);
      ...
      if (numberChecked === 0) {
        ...
        const c1 = indices[0] !== position ? optionsList[indices[0]] : optionsList[indices[1]];
        const c2 = indices[0] !== position ? optionsList[indices[1]] : optionsList[indices[0]];
        c = [c1, c2];
      }

      const format = c?.map(d => ({
        id: d.id,
        type: d.type
      }));
