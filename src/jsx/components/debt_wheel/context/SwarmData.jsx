...
import Metric_Context from './Metric';
...
export function SwarmDataContextProvider({ children }) {
  ...
  const lineData = useMemo(
    () =>
      valuesData
        ...
        .map(d => ({
          ...d,
          indicator_info: indicatorData.find(i => i.indicator === d.indicator),
          id_info: idData.find(i => i.id === d.id),
          class: d.id === id.id ? 'focus_line' : comparisons[0] && d.id === comparisons[0].id ? 'comparison_1_line' : comparisons[1] && d.id === comparisons[1].id ? 'comparison_2_line' : 'no_highlight_line'
        })),
      ...
      lineData
        ...
        .map(d => {
          ...
          return {
            ...
            class: d.id === id.id || focus ? 'focus_circle' : id.type !== 'country' ? 'no_highlight_circle' : one && two ? 'both_comparisons_circle' : one ? 'comparison_1_circle' : two ? 'comparison_2_circle' : 'no_highlight_circle'
          };
        }),
  ...
  const referenceLines = useMemo(
    () =>
      lineData
        ?.filter(d => +d.latest_year === 1 && ((comparisons[0] && comparisons[0].id === d.id) || (comparisons[1] && comparisons[1].id === d.id)))
        .map(d => ({
          ...d,
          class: comparisons[0] && comparisons[0].id === d.id ? 'comparison_1_line_thin' : 'comparison_2_line_thin'
        }))
  ...
  return <SwarmDataContext.Provider value={context}>{children}</SwarmDataContext.Provider>;
}
