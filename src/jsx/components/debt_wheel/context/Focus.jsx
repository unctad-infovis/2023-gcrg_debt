...
export function FocusContextProvider({ children }) {
  ...
  const [comparisons, setComparisons] = useState([{ type: 'development', id: 'Developed countries' }]);

  const [readURL, setReadURL] = useState(false);
  useEffect(() => {
    const query = window.location.href.includes('?') ? window.location.href.split('?')[1] : null;
    if (query) {
      ...
      const c1_query = pairs[1]?.split('=')[0] === 'comparison1' ? pairs[1].split('=')[1] : null;
      const c2_query = pairs[2]?.split('=')[0] === 'comparison2' ? pairs[2].split('=')[1] : null;

      if ((c1_query || c2_query) && idData) {
        const c1_data = idData.find(d => d.id === decodeURIComponent(c1_query));
        const c2_data = idData.find(d => d.id === decodeURIComponent(c2_query));
  ...
  const comparisonLists = useMemo(
    () => ({
      comparison_1: idData && comparisons[0] ? idData.filter(d => d[comparisons[0].type] === comparisons[0].id || +d[comparisons[0].type] === 1) : [],
      comparison_2: idData && comparisons[1] ? idData.filter(d => d[comparisons[1].type] === comparisons[1].id || +d[comparisons[1].type] === 1) : []
    }),
    [idData, comparisons]
  );

  const focusList = useMemo(() => (idData && id ? idData.filter(d => d[id.type] === id.id || +d[id.type] === 1) : []), [idData, id]);
    ...
    () =>
      idData
        ...
        .map((d, i) => ({
          ...d,
          order: i,
          checked: comparisons[0] && comparisons[0].id === d.id ? true : !!(comparisons[1] && comparisons[1].id === d.id)
        })),
    [idData, comparisons]
  );
